                    ...
                    <div class="-my-2 base:-mx-6 lg:-mx-8">
                        <div class="py-2 align-middle inline-block min-w-full base:px-6 lg:px-8">
                            <div class="shadow border-b border-gray-200 base:rounded-lg">
                                    ...
                                    <div class="flex-auto w-32">
                                        <div class="grid justify-items-stretch ...">
                                            <div class="justify-self-end ">
                                                <el-button type="primary" v-on:click="insertfilter('110')" size="large"
                                                    round>110年度</el-button>
                                                <el-button type="primary" v-on:click="insertfilter('111')" size="large"
                                                    round>111年度</el-button>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="flex-auto w-32">
                                        <div class="grid justify-items-stretch ...">
                                            <div class="justify-self-center ">
                                                <el-date-picker type="daterange" align="right" unlink-panels
                                                    v-model="value2" range-separator="至" start-placeholder="開始日期"
                                                    end-placeholder="結束日期" :shortcuts="shortcuts"
                                ...
                                <el-table :data="this.showData.filter((data) =>
                                    (!search || data.requisition_num.toLowerCase().includes(search.toLowerCase())) &&
                                    (!planfilter || data.plan_name.includes(planfilter))&& (new Date(data.created_at)).getTime() > this.value2[0].getTime() &&
                                    ...
                                    <el-table-column prop="plan_name" sortable label="經費來源" width="auto" align="left">
                                        <template #default="pc">
                                            <div class="text-gray-900">{{ pc.row.plan_name }} - {{ pc.row.fund_name }}
                                            </div>
                                    ...
                                    <el-table-column prop="department" :filters="school_dep" width="auto"
                                        :filter-method="filterdep" sortable label="申請人(申請處室)" align="center" />
                                    <el-table-column prop="requisition_num" width="auto" sortable label="請購單號"
                                        align="center" />
                                    <el-table-column prop="amount" sortable :min-width="70" width="auto" label="核銷金額"
                                        align="center">
                                    ...
                                    <el-table-column prop="status_id" width="auto" :min-width="53" sortable label="核銷狀態"
                                        ...
                                        <template #default="scope">
                                            <el-tag style="font-size: 18px" size="large" type="danger"
                                                disable-transitions v-if="scope.row.status_id === 1">未核銷</el-tag>
                                            <el-tag style="font-size: 18px" size="large" type="success"
                                                disable-transitions v-if="scope.row.status_id === 2">已核銷</el-tag>
                                        </template>
                                    </el-table-column>
                                    <el-table-column prop="created_at" width="auto" sortable label="備註"
                                        :formatter="DateFormat" />
                                    <el-table-column prop="name" :min-width="33" sortable label="建立人" align="center" />
                                    <el-table-column align="center" width="auto">
                                        <template #default="scope">
                                            <a :href="route('subplan.show', scope.row.sub_plan_fund_detail_id)"
                                                class="text-indigo-600 hover:text-indigo-900">檢視</a>
                                            <a :href="route('subplan.edit', scope.row.sub_plan_fund_detail_id)"
                                                class="ml-2 text-indigo-600 hover:text-indigo-900"
                                                v-if="scope.row.status_id == 1">修改</a>
                                            <a class="cursor-pointer ml-2 text-indigo-600 hover:text-indigo-900"
                                                @click="opencheck(scope.row.sub_plan_fund_detail_id)"
                                                v-if="scope.row.status_id == 1 && $page.props.user.role == 'admin'">核銷</a>
                                        </template>
                                    </el-table-column>
                                </el-table>
                            </div>
                                ...
                                <div class="fixed inset-0 z-10 overflow-y-auto">
                                    <div class="min-h-screen px-4 text-center">
                                        ...
                                        <TransitionChild as="template" enter="duration-300 ease-out"
                                            ...
                                            <div
                                                class="inline-block w-full max-w-sm p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl border-purple-400 border-4">
                                                <div class="flex items-center justify-between  justify-items-stretch">
                                                    <div
                                                        class="w-0 flex-1 justify-self-auto grid justify-items-stretch">
                                                        <span class="flex p-2 m-auto justify-self-center">
                                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                                class="w-14 h-14 text-red-600 justify-items-center"
                                                    ...
                                                    <div class="w-0 flex-1 grid justify-items-stretch">
                                                        <span class="flex p-2 m-auto justify-self-end">
                                                            <div class="m-2 flex">
                                                                ...
                                                                <div class="m-2 flex">
                                                                    <button type="button"
                                                                        class="inline-flex justify-center px-4 py-2 text-base text-white font-black bg-indigo-500 border border-transparent rounded-md hover:bg-indigo-400 focus-visible:ring-2 focus-visible:ring-offset-2"
                                                                        @click="checkwindow()">確定
                                                                    </button>
                                                                </div>
                                                            </div>
                                                        </span>
...
export default defineComponent({
    ...
    methods: {
        handleScroll() {
            if ((window.scrollY + window.screen.height >= document.body.scrollHeight) && this.showCount) {
                this.showCount = false
                let loc = window.scrollY - window.screen.height;
                let last = [this.showData[0].implementation_matter_name, this.showData[0].sub_strategy];
                this.res = [];
                let data = [];
                let count = 1;
                axios.get(route('scroll'), { params: { data_now: this.rowsCount, nowYear: this.planfilter } })
                    .then(res => {
                        this.showCount = true
