                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                          <div class="grid grid-cols-6 gap-6">
                            ...
                            <jet-validation-errors class="mb-4" />
                              <jet-label for="category" class="block text-sm font-medium text-gray-700">請購類別<a class="text-red-500">*</a> </jet-label>
                            ...
                            <div class="col-span-6 sm:col-span-3">
                              <jet-label for="requisition_num" class="block text-sm font-medium text-gray-700">單號<a class="text-red-500">*</a> </jet-label>
                              <input type="text" v-model="form.requisition_num" name="requisition_num" size="12" maxlength="12" id="requisition_num" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>
                            </div>

                            <div class="col-span-2">
                              <jet-label for="applicant" class="block text-sm font-medium text-gray-700">申請人<a class="text-red-500">*</a> </jet-label>
                            ...
                            <div class="col-span-2">
                              <jet-label for="amount" class="block text-sm font-medium text-gray-700">選擇處室<a class="text-red-500">*</a> </jet-label>
                                <div class="mt-1 relative">
                                  <input list="dep_name" type="text" @change="Getfundlist()" v-model="form.dep_name" name="dep_name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>
                                    <datalist id="dep_name" >
                                      <option v-for="dep in deps" :key="dep.dep_id" class="mt-1 hover:ring-indigo-500 hover:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                      {{dep.academy}}-{{dep.department}}
                                      </option>
                                    </datalist>
                                </div>
                            </div>
                            <div class="col-span-6 sm:col-span-3 lg:col-span-2">
                              <jet-label for="amount" class="block text-sm font-medium text-gray-700">核銷金額<a class="text-red-500">*</a> </jet-label>
                                  ...
                                  <Listbox as="div" v-model="selected">
                                    ...
                                    <div class="mt-1 relative">
                                      <ListboxButton class="relative w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"  v-model="form.academy">
                                          <span class="ml-3 block truncate" v-if="selected.plan_name">({{selected.sub_item_id}}-{{selected.sub_num}}){{ selected.letter_num }}-{{ selected.plan_name }}-{{ selected.fund_name }}({{selected.amount}})</span>
                                          ...
                                          <ListboxOption as="template" v-for="ll in fund_list" :key="ll.dep_id" :value="ll" v-slot="{ active, selected }">
                                            <li :class="[active ? 'text-white bg-indigo-600' : 'text-gray-900', 'cursor-default select-none relative py-2 pl-3 pr-9']">
                                              <div class="flex items-center">
                                                <span :class="[selected ? 'font-semibold' : 'font-normal', 'ml-3 block truncate']" v-if="ll.letter_num">
                                                  ({{ll.sub_item_id}}-{{ll.sub_num}}){{ ll.letter_num }}-{{ ll.plan_name }}-{{ ll.fund_name }}({{ll.amount}})
                                                </span>
                                              </div>
...
export default defineComponent({
        ...
        created() {
          ...
          this.selected = ref(this.fund_list[0])
        },
        methods: {
            createrequisition() {
              this.form.fund_detail_id= this.selected.fund_detail_id;
              this.form.post(this.route('subplan.store'))
            },
            Getfundlist(){
              let depid = this.deps.find(element => element.department == this.form.dep_name.split('-')[1]);
              if(depid){
                let  aa= new FormData;
